...
const fallbackResponse = {
  success: true,
  message: "Videos merged successfully (fallback response)",
  data: {
    videoMergeStatus: "completed",
    processingTime: 3.2  // time in seconds
  }
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Processing merge-videos request for session: ${session_id}`);
    ...
    return NextResponse.json(
      { success: false, error: "Failed to merge videos" },
      { status: 500 }
    );
  }
} 