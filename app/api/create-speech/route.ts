...
const fallbackResponse = {
  success: true,
  message: "Speech created successfully (fallback response)",
  data: {
    audioUrl: "https://example.com/sample-speech.mp3",
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Processing create-speech request for session: ${session_id}`);
    
    // Make the actual API call to your backend service
    try {
      const response = await fetch(`${API_BASE_URL}/create-speech`, {
        method: 'POST',
        headers: {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to create speech" },
      { status: 500 }
    );
  }
} 